...
int move(int height, int width, char maze[][width], int y, int x){
    bool searchN, searchE, searchS, searchW, moved = false, searched = false, deadend = false;
    int direction;
    ...
    if (deadend == false){
        ...
        maze[y][x] = ' ';
        int check = move(height, width, maze, y, x);
        if (check == 1){
